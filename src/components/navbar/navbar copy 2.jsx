...
const Navbar = () => {
  ...
  const renderUserMenu = () => (
    <DropdownMenu>
      <DropdownMenuTrigger
        className={`focus:outline-none outline-none focus:ring-0 cursor-pointer h-full ml-2 xl:ml-0 flex flex-row-reverse items-center gap-3 lg:py-2 lg:px-4 rounded-full lg:rounded-xl hover:shadow-md hover:shadow-blue/[7%] ${
          isDashboard ? "bg-white" : "bg-back2/30"
      ...
      <DropdownMenuTrigger asChild>
        <Tooltip title="Notifications" arrow>
          <button
            className={`relative ${
              isDashboard ? "bg-white" : "bg-back2/30"
            } hover:bg-muted-foreground/10 p-2 rounded-lg ease-in-out duration-200 hover:shadow-md hover:shadow-blue/[7%]`}
          >
            <Bell size={21} />
            {notifications &&
        ...
        <>
          ...
          <nav className="z-20 fixed md:hidden flex top-0 w-full h-20 ">
            <div className="w-full flex items-center justify-between px-6 transition-all ease-in-out duration-300">
              {/* Post button section */}
              {isPost && (
                <div className="flex items-center gap-2 xl:gap-3 text-black/80  backdrop-blur-lg py-2 px-6 rounded-xl">
                  <button
                    onClick={() => {
                    ...
                    className={`${
                      isPost ? "flex" : "hidden"
                    } gap-1 text-black bg-white text-sm items-center justify-center px-3 py-2  lg:mt-5 rounded-lg hover:bg-muted-foreground/10 duration-200`}
                  >
              ...
              {/* Menu button section */}
              <div
                className={`ml-1 h-12 w-12 flex items-center justify-center text-black/80 bg-primary-foreground/80 backdrop-blur-lg rounded-xl ${
                  isPost ? "hidden" : "flex"
                }`}
              >
                <button
                  className="text-blue text-3xl bg-transparent flex items-center justify-center"
          ...
          {/* Medium Screen */}
          <nav
            className={`z-20 sticky hidden md:flex top-0 container  mx-auto max-w-7xl px-10 md:px-5 `}
          >
            <div
              className={` absolute right-0 top-4 w-full flex items-center ${
                isPost ? "justify-between" : "justify-end"
              } px-10 md:px-0 transition-all ease-in-out duration-300 `}
            >
              {isPost && (
                <div className=" flex items-center gap-2  text-black/80 bg-primary-foreground/80 backdrop-blur-lg py-3 px-5 rounded-xl">
                  <button
                    onClick={() => {
                      if (sessionStorage.getItem("savedStep") !== "") {
                        setDiscardModal(true);
                    ...
                    className={`${
                      isPost ? "flex" : "hidden"
                    } gap-1 text-sm items-center justify-center text-black bg-white px-3 py-2  rounded-lg hover:bg-muted-foreground/10 duration-200`}
                  >
                    <FaChevronLeft className="text-xs pb-0.5" /> Dashboard
                  </button>
                </div>
              )}
              <div className="flex items-center gap-2 xl:gap-3 text-black/80 bg-primary-foreground/80 backdrop-blur-lg py-3 px-5 rounded-xl">
                {/* Uncomment message button */}
                {!isPost &&
                  (roleType === "client" || !isLoggedIn ? (
                    <Button
                      onClick={() =>
            ...
            <div className="flex items-center justify-between w-full mx-auto max-w-7xl h-20 px-10 md:px-5">
              <div className="h-full flex">
                <img
                  ...
                  onClick={() => navigate("/")}
                />
                <div className="flex items-center gap-4 ml-6 text-sm  text-black">
                  <span
